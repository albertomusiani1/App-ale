 ...
 * - generic-> uscite, avventure e qualsiasi categoria creata da voi
 * - goals  -> achievement
 */
 ...
 * quelli manuali li spuntate voi.
 */
export interface Achievement {
  ...
  description: string
  emoji: string
  /** 'auto' = calcolato dai dati, 'manual' = lo spuntate voi. */
  kind: 'auto' | 'manual'
  /** Quante cose servono per sbloccarlo (per quelli automatici). */
  target: number
  unlockedAt: string | null
  /** true per quelli che avete creato voi. */
  custom: boolean
}

/** Un vostro modo di dire, da non dimenticare. */
